...
with conn, conn.cursor() as cur:
    print(f"Processing and embedding '{pdf_path}' in batches of {batch_size}...")
    ...
    for page_num, chunk in chunks_from_pdf(pdf_path):
        ...
        # For the COPY command, pgvector expects the vector as a string.
        ...
        if len(batch) >= batch_size:
            ...
            batch = []
            print(f"Committed a batch of {batch_size} chunks.")
    ...
    if batch:
        with cur.copy("COPY docs (id, pdf_id, page, text, embedding) FROM STDIN") as copy:
            for record in batch:
                copy.write_row(record)
        print(f"Committed the final batch of {len(batch)} chunks.")
